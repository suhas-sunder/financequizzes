...
import {
  buildCanonicalUrl,
  getSiteUrlFromEnv,
} from "../client/components/budgeting-quiz/seoCanonical.server";
import type { Route } from "./+types/budgeting-quiz";
import { useLoaderData } from "react-router";
import { useEffect, useMemo, useRef, useState } from "react";

import {
  BUDGETING_QUIZ_QUESTION_BANK,
  BUDGETING_FAQS,
  type QuizQuestion,
} from "../client/components/budgeting-quiz/budgetingQuizData";

import { Breadcrumbs } from "../client/components/budgeting-quiz/Breadcrumbs";
import { QuizQuestions } from "../client/components/budgeting-quiz/QuizQuestions";
import { BudgetingArticle } from "../client/components/budgeting-quiz/BudgetingArticle";
import { FAQSection } from "../client/components/budgeting-quiz/FAQSection";
import { BudgetingQuizJsonLd } from "../client/components/budgeting-quiz/BudgetingQuizJsonLd";
...
export function meta({ data }: Route.MetaArgs) {
  const title =
    "Budgeting Quiz (10 Questions) | FinanceQuizzes.com";

  const description =
    "Test your budgeting fundamentals in under 3 minutes. A free 10-question quiz covering budgeting concepts, categories, allocations, and planning vs tracking terms.";

  const url =
    data?.canonicalUrl ?? "https://www.financequizzes.com/budgeting-quiz";

  const ogImage =
    data?.ogImageUrl ?? "https://www.financequizzes.com/og/budgeting-quiz.png";
  ...
  return [
    ...
    { property: "og:image:alt", content: "Budgeting Quiz with 10 Questions" },
    ...
    { name: "twitter:image:alt", content: "Budgeting Quiz with 10 Questions" },
...
const PERFECT_COUNT_STORAGE_KEY = "financequizzes:perfectCount:/budgeting-quiz";
const BEST_TIME_STORAGE_KEY = "financequizzes:bestTimeMs:/budgeting-quiz";
...
export default function BudgetingQuiz({}: Route.ComponentProps) {
  const { faqs, canonicalUrl } = useLoaderData() as LoaderData;

  const questionBank = BUDGETING_QUIZ_QUESTION_BANK;
  ...
  return (
    <main className="bg-white text-[#0B1B2B]">
      ...
      >
        <Breadcrumbs currentLabel="Budgeting Quiz" />
          ...
          >
            <header className="mb-2">
              ...
              <p className="mt-2 text-center text-sm sm:text-base text-slate-600">
                A quick 10-question quiz focused on budgeting terminology,
                structures, and planning concepts.
              </p>
            </header>
      ...
      <BudgetingQuizJsonLd faqs={faqs} pageUrl={pageUrl} />
    </main>
  );
}
