...
import {
  buildCanonicalUrl,
  getSiteUrlFromEnv,
} from "../client/components/personal-finance-quiz/seoCanonical.server";
import type { Route } from "./+types/personal-finance-quiz";
import { useEffect, useMemo, useRef, useState } from "react";

import {
  PERSONAL_FINANCE_QUIZ_QUESTION_BANK,
  PERSONAL_FINANCE_FAQS,
  type QuizQuestion,
} from "../client/components/personal-finance-quiz/personalFinanceQuizData";

import { Breadcrumbs } from "../client/components/personal-finance-quiz/Breadcrumbs";
import { QuizQuestions } from "../client/components/personal-finance-quiz/QuizQuestions";
import { PersonalFinanceArticle } from "../client/components/personal-finance-quiz/PersonalFinanceArticle";
import { FAQSection } from "../client/components/personal-finance-quiz/FAQSection";
import { PersonalFinanceQuizJsonLd } from "../client/components/personal-finance-quiz/PersonalFinanceQuizJsonLd";
...
export function meta({ data }: Route.MetaArgs) {
  const title = "Personal Finance Quiz | FinanceQuizzes.com";
  const description =
    "Fast, interactive personal finance quiz in a 10-question challenge. Budgeting, saving, debt, credit cards, bills, and everyday money decisions.";
  const url =
    data?.canonicalUrl ??
    "https://www.financequizzes.com/personal-finance-quiz";
...
const PERFECT_COUNT_STORAGE_KEY =
  "financequizzes:perfectCount:/personal-finance-quiz";
const BEST_TIME_STORAGE_KEY =
  "financequizzes:bestTimeMs:/personal-finance-quiz";
...
function scoreMessage(correctCount: number) {
  // 10 questions.
  if (correctCount <= 0) return "New run. Try again with a fresh set.";
  if (correctCount >= 10) return "Perfect score.";
  if (correctCount >= 8) return "Strong everyday money knowledge.";
  if (correctCount >= 6) return "Good understanding overall.";
  if (correctCount === 5)
    return "Average familiarity. A few gaps to tighten up.";
  if (correctCount >= 4) return "You have the basics, but review key terms.";
  return "You’re building familiarity. Keep going.";
}
...
export default function PersonalFinanceQuiz({}: Route.ComponentProps) {
  ...
  const questionBank = PERSONAL_FINANCE_QUIZ_QUESTION_BANK;
  ...
  // Synchronous guard to prevent double-answering from rapid taps/clicks before state updates.
  ...
  return (
    <main className="bg-white text-[#0B1B2B]">
      ...
      >
        <Breadcrumbs currentLabel="Personal Finance Quiz" />
        ...
        <div className="mx-auto max-w-6xl text-center">
          ...
          >
            <header className="mb-2">
              <h1 className="text-center text-3xl sm:text-3xl font-extrabold tracking-tight text-slate-700">
                Personal Finance Quiz
              </h1>
              <p className="mt-2 text-center text-sm sm:text-base text-slate-600">
                10 questions on budgeting, saving, debt, credit, and everyday
                money decisions.
              </p>
            </header>
            ...
            {poolTooSmall ? (
              ...
              <QuizQuestions
                quizTitle="Personal Finance Quiz"
      ...
      <PersonalFinanceArticle />

      <FAQSection faqs={faqs} />
      <PersonalFinanceQuizJsonLd faqs={faqs} pageUrl={pageUrl} />
    </main>
  );
}
