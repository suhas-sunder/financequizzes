...
import { buildCanonicalUrl, getSiteUrlFromEnv } from "../client/components/business-finance-quiz/seoCanonical.server";
import type { Route } from "./+types/business-finance-quiz";
import { useLoaderData } from "react-router";
import { useEffect, useMemo, useRef, useState } from "react";

import {
  BUSINESS_FINANCE_QUIZ_QUESTION_BANK,
  BUSINESS_FINANCE_FAQS,
  type QuizQuestion,
} from "../client/components/business-finance-quiz/businessFinanceQuizData";

import { Breadcrumbs } from "../client/components/business-finance-quiz/Breadcrumbs";
import { QuizQuestions } from "../client/components/business-finance-quiz/QuizQuestions";
import { BusinessFinanceArticle } from "../client/components/business-finance-quiz/BusinessFinanceArticle";
import { FAQSection } from "../client/components/business-finance-quiz/FAQSection";
import { BusinessFinanceQuizJsonLd } from "../client/components/business-finance-quiz/BusinessFinanceQuizJsonLd";
...
export function meta({ data }: Route.MetaArgs) {
  const title =
    "Business Finance Quiz (10 Questions) | FinanceQuizzes.com";

  const description =
    "Test your business finance fundamentals in under 3 minutes. A free 10-question quiz covering revenue vs profit, cash flow, costs, margins, break-even, and core financial ratios.";

  const url =
    data?.canonicalUrl ??
    "https://www.financequizzes.com/business-finance-quiz";

  const ogImage =
    data?.ogImageUrl ??
    "https://www.financequizzes.com/og/business-finance-quiz.png";
  ...
  return [
    ...
    { property: "og:image:alt", content: "Business Finance Quiz with 10 Questions" },
    ...
    { name: "twitter:image:alt", content: "Business Finance Quiz with 10 Questions" },
...
const PERFECT_COUNT_STORAGE_KEY = "financequizzes:perfectCount:/business-finance-quiz";
const BEST_TIME_STORAGE_KEY = "financequizzes:bestTimeMs:/business-finance-quiz";
...
export default function FinanceQuiz({}: Route.ComponentProps) {
  ...
  const questionBank = BUSINESS_FINANCE_QUIZ_QUESTION_BANK;
  ...
  useEffect(() => {
    ...
    if (!isComplete && stats.answered === QUIZ_QUESTION_COUNT) {
      ...
      if (!hasCountedPerfectRef.current && stats.correct === QUIZ_QUESTION_COUNT) {
  ...
  return (
    <main className="bg-white text-[#0B1B2B]">
      ...
      >
        <Breadcrumbs currentLabel="Business Finance Quiz" />
          ...
          >
            <header className="mb-2">
              ...
              <p className="mt-2 text-center text-sm sm:text-base text-slate-600">
                A quick 10-question quiz on business finance language: revenue vs profit,
                cash flow, costs, margins, break-even, and ratios.
              </p>
            </header>
      ...
      <BusinessFinanceArticle />

      <FAQSection faqs={faqs} />

      <BusinessFinanceQuizJsonLd faqs={faqs} pageUrl={pageUrl} />
    </main>
  );
}
