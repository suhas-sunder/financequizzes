...
import { buildCanonicalUrl, getSiteUrlFromEnv } from "../client/components/saving-money-quiz/seoCanonical.server";
import type { Route } from "./+types/saving-money-quiz";
import { useLoaderData } from "react-router";
import { useEffect, useMemo, useRef, useState } from "react";

import {
  SAVING_MONEY_QUIZ_QUESTION_BANK,
  SAVING_MONEY_FAQS,
  type QuizQuestion,
} from "../client/components/saving-money-quiz/savingMoneyQuizData";

import { Breadcrumbs } from "../client/components/saving-money-quiz/Breadcrumbs";
import { QuizQuestions } from "../client/components/saving-money-quiz/QuizQuestions";
import { SavingMoneyArticle } from "../client/components/saving-money-quiz/SavingMoneyArticle";
import { FAQSection } from "../client/components/saving-money-quiz/FAQSection";
import { SavingMoneyQuizJsonLd } from "../client/components/saving-money-quiz/SavingMoneyQuizJsonLd";
...
export function meta({ data }: Route.MetaArgs) {
  const title =
    "Saving Money Quiz (10 Questions) | FinanceQuizzes.com";

  const description =
    "Test your saving fundamentals in under 3 minutes. A free 10-question quiz covering goals, habits, tradeoffs, and time-oriented saving decisions.";

  const url =
    data?.canonicalUrl ??
    "https://www.financequizzes.com/saving-money-quiz";

  const ogImage =
    data?.ogImageUrl ??
    "https://www.financequizzes.com/og/saving-money-quiz.png";
  ...
  return [
    ...
    { property: "og:image:alt", content: "Saving Money Quiz with 10 Questions" },
    ...
    { name: "twitter:image:alt", content: "Saving Money Quiz with 10 Questions" },
...
const PERFECT_COUNT_STORAGE_KEY = "financequizzes:perfectCount:/saving-money-quiz";
const BEST_TIME_STORAGE_KEY = "financequizzes:bestTimeMs:/saving-money-quiz";
...
export default function FinanceQuiz({}: Route.ComponentProps) {
  ...
  const questionBank = SAVING_MONEY_QUIZ_QUESTION_BANK;
  ...
  return (
    <main className="bg-white text-[#0B1B2B]">
      ...
      >
        <Breadcrumbs currentLabel="Saving Money Quiz" />
          ...
          >
            <header className="mb-2">
              ...
              <p className="mt-2 text-center text-sm sm:text-base text-slate-600">
                A quick 10-question quiz focused on saving concepts like goals, habits,
                tradeoffs, and time orientation.
              </p>
            </header>
      ...
      <SavingMoneyQuizJsonLd faqs={faqs} pageUrl={pageUrl} />
    </main>
  );
}
