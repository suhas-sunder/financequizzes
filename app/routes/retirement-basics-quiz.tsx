...
import { buildCanonicalUrl, getSiteUrlFromEnv } from "../client/components/retirement-basics-quiz/seoCanonical.server";
import type { Route } from "./+types/retirement-basics-quiz";
import { useLoaderData } from "react-router";
import { useEffect, useMemo, useRef, useState } from "react";

import {
  RETIREMENT_BASICS_QUIZ_QUESTION_BANK,
  RETIREMENT_BASICS_FAQS,
  type QuizQuestion,
} from "../client/components/retirement-basics-quiz/retirementBasicsQuizData";

import { Breadcrumbs } from "../client/components/retirement-basics-quiz/Breadcrumbs";
import { QuizQuestions } from "../client/components/retirement-basics-quiz/QuizQuestions";
import { RetirementBasicsArticle } from "../client/components/retirement-basics-quiz/RetirementBasicsArticle";
import { FAQSection } from "../client/components/retirement-basics-quiz/FAQSection";
import { RetirementBasicsQuizJsonLd } from "../client/components/retirement-basics-quiz/RetirementBasicsQuizJsonLd";
...
export function meta({ data }: Route.MetaArgs) {
  const title =
    "Retirement Basics Quiz (10 Questions) | FinanceQuizzes.com";

  const description =
    "Test your retirement planning fundamentals in under 3 minutes. A free 10-question quiz covering purpose, time horizons, goals, and high-level planning concepts.";

  const url =
    data?.canonicalUrl ??
    "https://www.financequizzes.com/retirement-basics-quiz";

  const ogImage =
    data?.ogImageUrl ??
    "https://www.financequizzes.com/og/retirement-basics-quiz.png";
  ...
  return [
    ...
    { property: "og:image:alt", content: "Retirement Basics Quiz with 10 Questions" },
    ...
    { name: "twitter:image:alt", content: "Retirement Basics Quiz with 10 Questions" },
...
const PERFECT_COUNT_STORAGE_KEY = "financequizzes:perfectCount:/retirement-basics-quiz";
const BEST_TIME_STORAGE_KEY = "financequizzes:bestTimeMs:/retirement-basics-quiz";
...
export default function FinanceQuiz({}: Route.ComponentProps) {
  ...
  const questionBank = RETIREMENT_BASICS_QUIZ_QUESTION_BANK;
  ...
  return (
    <main className="bg-white text-[#0B1B2B]">
      ...
      >
        <Breadcrumbs currentLabel="Retirement Basics Quiz" />
          ...
          >
            <header className="mb-2">
              ...
              <p className="mt-2 text-center text-sm sm:text-base text-slate-600">
                A quick 10-question quiz on retirement concepts: purpose, time horizons,
                and long-term planning framing.
              </p>
            </header>
      ...
      <RetirementBasicsArticle />

      <FAQSection faqs={faqs} />

      <RetirementBasicsQuizJsonLd faqs={faqs} pageUrl={pageUrl} />
    </main>
  );
}
